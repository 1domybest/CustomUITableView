...
//               cell.circleButton.addTarget(self, action: #selector(subscribeTapped(_:)), for: .touchUpInside)
...
struct LiveStreamingMessageViewWrapper: UIViewRepresentable {
    var data: [MessageRenderingProtocol]
    
    func makeUIView(context: Context) -> LiveStreamingMessageView {
        let tableView = LiveStreamingMessageView(frame: .zero, style: .plain)
        tableView.data = data
        return tableView
    }
    
    func updateUIView(_ uiView: LiveStreamingMessageView, context: Context) {
        uiView.updateData(data)
    }
}

class LiveStreamingMessageView: UITableView, UITableViewDataSource, UITableViewDelegate {
    ...
    override init(frame: CGRect, style: UITableView.Style) {
        ...
        dataSource = self
        delegate = self
        register(ChatMessageWithButtonCell.self, forCellReuseIdentifier: "id")
        separatorStyle = .none
    }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        let maxWidth = UIScreen.main.bounds.width - (10 * 2) - (12 * 2)
        let maxHeight = ceil(messageWidth/maxWidth) * messageHeight
        
        return maxHeight + (10 * 2) + (6 * 2)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.circleButton.isUserInteractionEnabled = true
        cell.circleButton.tag = indexPath.row
        cell.circleButton.addTarget(self, action: #selector(subscribeTapped(_:)), for: .touchUpInside)

        
        print("선택 가능? \(cell.circleButton.isEnabled)")
        cell.selectedBackgroundView = UIView()
